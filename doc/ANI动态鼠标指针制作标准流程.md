...
> **现行动画制作（2026-08 起）**  
> 高清 concept 确认后，用 Seedance 图生视频，抽 **16 帧** 做成 **128×128 / 32 位** ANI。  
> 完整步骤、提示词和验收见 [`Seedance动画指针制作流程.md`](Seedance动画指针制作流程.md)。  
> 本文保留交付物、17 角色表、热点规则和 ANI 封装校验。  
> **不要再走 Prompt B / 逐像素拆层 / `build_assets.py` 猜层派生续帧。**

本文说明如何把帧封装成由 Windows 原生播放的动态鼠标指针，并导入 QMcursor。目标是关闭 QMcursor 后仍能继续播放，不依赖托盘、叠加层或后台进程。
...
- 角色态循环（现行）：**16 帧**
- 忙碌或后台运行：16 帧；动作可更强，仍用同一封装
- 功能态：可用 1 帧或 16 帧相同静帧
- 默认帧间隔：约 100ms
- ANI `JifRate`：6，即 `6 / 60` 秒
- 现行循环时长：约 1600ms（16×100ms）
...
- 母版来自已确认 concept 的 Seedance 短视频
- 抽 16 帧后 LANCZOS 装进 **128×128**
- ANI 使用 **32 位 RGBA**
- 细节见 [`Seedance动画指针制作流程.md`](Seedance动画指针制作流程.md)

#### 已停用：真像素手绘 / 8 位派生

- 不再按 32/48 逻辑格手绘正式帧
- 不再用 8 位索引色交差（高清缩小后颜色会超）
